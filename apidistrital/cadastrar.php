...
if($postjson['cep_usuario'] == ''){
  $result = json_encode(array('success'=>'Preencha todos os campos!'));
  echo $result;
  exit();
}
if($postjson['endereco_usuario'] == ''){
  $result = json_encode(array('success'=>'CepUser!'));
  echo $result;
  exit();
}

if($postjson['igreja_distrito'] == ''){
  $result = json_encode(array('success'=>'Preencha a qual igreja você pertence!'));
  echo $result;
  exit();
}
if($postjson['nivel'] == ''){
  $result = json_encode(array('success'=>'Preencha a opção!'));
  echo $result;
  exit();
}
 ...
 $query_buscar = $pdo->query("SELECT * from usuarios where email = '$postjson[email]' OR  username = '$postjson[username]' OR rg_usuario = '$postjson[rg_usuario]' ");
