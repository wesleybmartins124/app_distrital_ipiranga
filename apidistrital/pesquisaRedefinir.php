...
$postjson = json_decode(file_get_contents("php://input"), true);
 ...
 $query_buscar = $pdo->query("SELECT * from usuarios where email = '$postjson[email]' and rg_usuario = '$postjson[rg_usuario]' ");
 ...
 for ($i=0; $i < count($dados_buscar); $i++) { 
   ...
   $dados = array(
	 ...
	 'lat' => $dados_buscar[$i]['lat'],
	 'lon' => $dados_buscar[$i]['lon'],
