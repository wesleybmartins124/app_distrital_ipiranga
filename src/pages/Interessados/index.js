...
import ListItem from '../../components/List/interessados';
...
export default function Interessados({navigation}) {
  ...
  async function listarDados(){
    const res = await axios.get(api + 'listarInteressados.php?busca=' + buscar);
...
async function deletar(id){
  ...
  const res = await axios.get(api + 'excluirInteressado.php?id=' + id);
  listarDados();
}
...
function editar(id){
  ...
  navigation.navigate('editarInt', {id: id});
    ...
    return (
      ...
      <View 
      style={styles.header}>
      <Text style={{color:'#FFF', fontSize:17}}>INSERIR PESSOA INTERESSADA</Text>
