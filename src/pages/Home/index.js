...
import { StyleSheet, Button, Text, View, SafeAreaView, TextInput, Image, Alert} from 'react-native';
...
function PostDel(id){
  if (dados.rg_usuario === '00.016.082-6'){
  Alert.alert(
    "Excluir Registro",
...
async function deletar(id){
  ...
  const res = await axios.get(api + 'excluirPublicacao.php?id=' + id);
  listarPost();
}
 ...
 style={[styles.imagenphoto]}
      ...
      <FlatList
        ...
        >
         ...
         <FabButton style={{bottom: 100, right:55}} pedido={() => navigation.navigate('addProgramacao')} Interessados={() => navigation.navigate('Interessados')}  oracao={() => navigation.navigate('oracao')} visualizarMembros={() => navigation.navigate('visualizarMembros')} NewPost={() => navigation.navigate('NewPost')}>
      </FabButton>
